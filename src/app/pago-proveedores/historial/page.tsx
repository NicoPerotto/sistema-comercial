        ...
        <main className="page-container">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <Link href="/pago-proveedores" className="p-1 hover:bg-surface rounded-lg transition-colors">
                ...
                <div className="flex gap-3">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="pl-10 pr-4 py-2 rounded-xl border border-border bg-background outline-none focus:ring-2 focus:ring-primary w-64 text-sm"
                            value={searchTerm}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="card-premium">
                    <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest mb-1">Total Histórico</p>
                ...
                <div className="card-premium border-l-4 border-l-primary">
                    <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest mb-1">Pagado este mes</p>
                ...
                <div className="card-premium border-l-4 border-l-orange-500">
                    <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest mb-1">Salida de Caja Hoy</p>
                            ...
                            {loading ? (
                                ...
                                <tr key={p.id} className="table-row">
                                    <td className="px-6 py-4">
                                        <span className="font-mono text-[10px] font-bold bg-surface px-2 py-1 rounded text-text-muted text-text-muted border border-border">
                                            {p.shortId}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[10px] text-text-muted">{new Date(p.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="font-bold text-sm">{p.supplier.name}</div>
                                        {p.description && <div className="text-[10px] text-text-muted truncate max-w-xs">{p.description}</div>}
                                    </td>
                                    <td className="px-6 py-4">
                                    ...
                                    <td className="px-6 py-4">
                                        {p.paidFromCash ? (
                                            <span className={`text-[9px] font-black px-1.5 py-0.5 rounded border flex items-center gap-1 w-fit ${p.cashRegister?.shortId === activeRegister?.shortId ? 'text-orange-600 bg-orange-50 border-orange-100' : 'text-text-muted bg-surface-alt border-border-subtle'}`}>
                                    ...
                                    <td className="px-6 py-4 text-right pr-6 font-black text-foreground">
