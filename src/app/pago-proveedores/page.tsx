        ...
        <main className="page-container">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-12">
                ...
                <div className="card-premium h-fit border-2 border-primary/10">
                    <div className="flex items-start gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-text-muted">Utiliza este módulo para rendir pagos realizados a proveedores. Asegúrate de vincular el código de caja si el dinero salió de una recaudación.</p>
                        </div>
                    </div>

                    <div className="space-y-4">
                        <div className="flex items-center gap-3 p-4 rounded-xl bg-surface-alt border border-border-subtle text-sm">
                    ...
                    <button
                        ...
                        className="w-full mt-8 py-4 rounded-xl bg-primary text-white font-black hover:scale-[1.02] transition-all flex items-center justify-center gap-2 group"
                    >
                ...
                <div className="card-premium h-fit">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="font-bold flex items-center gap-2 text-text-muted uppercase text-xs tracking-widest">
                    ...
                    <div className="space-y-4">
                        {loading ? (
                            Array(3).fill(0).map((_, i) => (
                                <div key={i} className="h-16 bg-surface animate-pulse rounded-xl"></div>
                            ))
                        ) : recentPayments.map(p => (
                            <div key={p.id} className="flex justify-between items-center p-4 rounded-xl border border-border-subtle bg-card shadow-sm">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-text-muted font-mono font-bold tracking-tighter">{p.shortId}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-sm font-black text-foreground">-${Number(p.amount).toLocaleString()}</p>
                                    <p className="text-[10px] text-text-muted font-medium">{new Date(p.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                                </div>
                            </div>
                        ))}
                        {recentPayments.length === 0 && !loading && (
                            <div className="text-center py-12 px-6 border-2 border-dashed border-border-subtle rounded-3xl">
                                <FileText className="w-10 h-10 text-foreground mx-auto mb-3" />
                                <p className="text-xs text-text-muted italic">No tienes pagos registrados en el historial cercano.</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-card rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 border border-white/20">
                        <header className="p-8 border-b border-border-subtle flex justify-between items-center bg-surface-alt/50 bg-surface-alt/50">
                            <div>
                                <h2 className="text-2xl font-black tracking-tight">Nuevo Egreso</h2>
                                <p className="text-xs text-text-muted uppercase font-black tracking-widest mt-1">Formulario de Pago</p>
                            </div>
                            <button onClick={() => setIsModalOpen(false)} className="w-10 h-10 rounded-full hover:bg-surface flex items-center justify-center transition-colors">
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-foreground">Monto</label>
                                    <div className="relative group">
                                        <span className="absolute left-5 top-1/2 -translate-y-1/2 font-black text-text-muted transition-colors group-focus-within:text-primary">$</span>
                                        <input
                                            required
                                            type="number"
                                            step="0.01"
                                            placeholder="0.00"
                                            className="w-full pl-10 pr-5 py-3.5 rounded-2xl border border-border outline-none focus:ring-4 focus:ring-primary/10 transition-all font-black text-xl"
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-foreground">Referencia / Nota</label>
                                    <input
                                        type="text"
                                        placeholder="Ej: Factura A-001..."
                                        className="w-full px-5 py-3.5 rounded-2xl border border-border outline-none focus:ring-4 focus:ring-primary/10 transition-all font-medium"
                            ...
                            {isArqueableSelection && (
                                <div className={`flex flex-col md:flex-row gap-6 p-6 rounded-3xl border transition-all ${activeRegister?.status !== 'OPEN' ? 'bg-surface-alt/20 border-border' : 'bg-orange-50/50 dark:bg-orange-900/10 border-orange-100/50 dark:border-orange-900/20'}`}>
                                    <div className="flex items-center gap-4 flex-1">
                                        <div
                                            className={`w-14 h-7 rounded-full transition-all relative ${activeRegister?.status !== 'OPEN' ? 'bg-surface cursor-not-allowed' : (formData.paidFromCash ? 'bg-orange-500 cursor-pointer' : 'bg-border cursor-pointer')}`}
                                            onClick={() => {
                                        ...
                                        >
                                            <div className={`absolute top-1 w-5 h-5 rounded-full bg-card shadow-sm transition-all ${formData.paidFromCash ? 'left-8' : 'left-1'}`}></div>
                                        </div>
                                        <div>
                                            <p className={`text-sm font-black leading-tight ${activeRegister?.status !== 'OPEN' ? 'text-text-muted' : 'text-orange-900 dark:text-orange-400'}`}>
                                    ...
                                    <div className="space-y-1 flex-1">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-text-muted">Código de Caja Vinculada</label>
                                        <input
                                            type="text"
                                            placeholder="EJ: AB123"
                                            disabled={formData.paidFromCash}
                                            className={`w-full px-4 py-2.5 rounded-xl border outline-none focus:ring-4 focus:ring-orange-500/10 text-sm font-black font-mono uppercase transition-all ${formData.paidFromCash ? 'bg-surface-alt border-border text-text-muted' : 'bg-card border-orange-200 text-orange-600 shadow-sm shadow-orange-500/5'}`}
