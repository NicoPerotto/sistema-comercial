        ...
        <main className="page-container">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1 max-w-sm">
                    ...
                    <input
                        ...
                        className="pl-9 pr-4 py-2 text-sm w-full bg-surface-alt border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                    />
                    {search && (
                        <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted hover:text-foreground">
                            <X className="w-3.5 h-3.5" />
                        </button>
                    )}
                </div>
                <div className="flex bg-surface rounded-lg p-1 border border-border gap-0.5">
                    {([['ALL', 'Todos'], ['COMPLETED', 'Completados'], ['CANCELLED', 'Cancelados']] as const).map(([val, label]) => (
                        <button
                            key={val}
                            onClick={() => setStatusFilter(val)}
                            className={`px-3 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all ${statusFilter === val ? 'bg-primary text-white shadow-sm' : 'text-text-faint hover:text-foreground'}`}
                        >
                            {label}
                        </button>
                    ))}
            ...
            <div className="card-premium p-0 overflow-hidden">
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                filtered.map((sale, idx) => {
                                    ...
                                    return (
                                        <tr key={sale.id} className={`table-row ${idx % 2 !== 0 ? 'bg-surface-alt/50' : ''}`}>
                                            {/* ID + Fecha */}
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-[9px] text-text-faint uppercase tracking-tighter">{sale.user?.role || '—'}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                {sale.paymentMethod ? (
                                                    <div className="inline-flex flex-col items-center gap-0.5">
                                                        <span className="text-[10px] font-bold text-foreground">{sale.paymentMethod.name}</span>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                {removedCount > 0 ? (
                                                    ...
                                                    <span className="text-[9px] font-medium text-text-faint">Limpio</span>
                                                )}
                                            </td>
                ...
                {!loading && filtered.length > 0 && (
                    <div className="px-6 py-3 border-t border-border-subtle bg-surface-alt flex items-center justify-between text-xs text-text-faint">
                        <span>{filtered.length} registro{filtered.length !== 1 ? 's' : ''} encontrado{filtered.length !== 1 ? 's' : ''}</span>
                        <span>
                            Total cobrado (completados): <span className="font-black text-foreground text-sm ml-1">${totalVisible.toLocaleString()}</span>
                        </span>
                    </div>
                )}
            </div>
                    ...
                    <div className="bg-card rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                        <header className="p-6 border-b border-border bg-surface-alt flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-[10px] text-text-faint font-mono mt-1">ID: {selectedSale.id}</p>
                            </div>
                        ...
                        <div className="p-6 space-y-6">
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <div className="p-4 bg-surface-alt rounded-xl border border-border">
                                    <p className="text-[10px] font-bold text-text-faint uppercase mb-1">Método</p>
                                    <p className="text-base font-bold">{selectedSale.paymentMethod?.name || 'Efectivo'}</p>
                                </div>
                                <div className="p-4 bg-surface-alt rounded-xl border border-border">
                                    <p className="text-[10px] font-bold text-text-faint uppercase mb-1">Tipo</p>
                            ...
                            <div className="overflow-y-auto max-h-64 space-y-4 pr-1 custom-scrollbar">
                                <section>
                                    <h4 className="text-[10px] font-bold uppercase tracking-widest text-text-faint mb-2">Artículos en Carrito</h4>
                                ...
                                {selectedSale.auditLog && (
                                    <section>
                                        <h4 className="text-[10px] font-bold uppercase tracking-widest text-warning mb-2 flex items-center gap-1">
                                            <AlertTriangle className="w-3 h-3" />
