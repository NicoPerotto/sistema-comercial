...
export default function CajaPage() {
    ...
    if (loading) return (
        <div className="p-10 flex flex-col gap-6 animate-pulse">
            ...
            <div className="h-64 bg-surface rounded-2xl max-w-xl"></div>
        </div>
    );
                ...
                <div className="max-w-xl animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="card-premium relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4 mb-8">
                            <div className="w-12 h-12 bg-success-subtle text-success-dark rounded-xl flex items-center justify-center">
                                <Calendar className="w-6 h-6" />
                            </div>
                            <div>
                                <h2 className="text-xl font-bold">Apertura de Caja</h2>
                                <p className="text-sm text-text-muted">Fondo inicial para comenzar el turno</p>
                            </div>
                        </div>

                        <form onSubmit={handleOpen} className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-foreground">Monto Inicial (Efectivo)</label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        placeholder="0.00"
                                        className="w-full bg-surface-alt border border-border rounded-xl p-4 pl-10 text-3xl font-bold text-primary focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all"
                                    />
                                </div>
                                {suggestedOpening > 0 && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="bg-foreground rounded-2xl p-8 text-background shadow-xl relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-6">
                            <div>
                                <h2 className="text-sm font-bold text-text-muted uppercase tracking-wider">Sesión Activa</h2>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                            <div className="bg-card/5 p-4 rounded-xl border border-white/10">
                                <p className="text-xs text-text-muted mb-1">Responsable</p>
                                <p className="font-bold">{registerData.openedBy?.name}</p>
                            </div>
                            <div className="bg-card/5 p-4 rounded-xl border border-white/10">
                                <p className="text-xs text-text-muted mb-1">Hora de Apertura</p>
                                <p className="font-bold">{new Date(registerData.openedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                            </div>
                        </div>

                        <div className="bg-card/5 p-6 rounded-xl border border-white/10">
                            <p className="text-sm text-text-muted mb-2">Fondo Inicial</p>
                    ...
                    <div className="card-premium relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4 mb-8">
                            <div className="w-12 h-12 bg-danger-subtle text-danger-dark rounded-xl flex items-center justify-center">
                                <ArrowDownToLine className="w-6 h-6" />
                            </div>
                            <div>
                                <h2 className="text-xl font-bold">Cierre de Caja</h2>
                                <p className="text-sm text-text-muted">Conteo de efectivo y arqueo final</p>
                            </div>
                        </div>

                        <form onSubmit={handleClose} className="space-y-6">
                            <div className="bg-primary-subtle p-4 rounded-xl text-primary-dark text-sm">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setAmount(e.target.value)}
                                        placeholder="0.00"
                                        className="w-full bg-surface-alt border border-border rounded-xl p-4 pl-10 text-3xl font-bold outline-none focus:ring-2 focus:ring-danger"
                                    />
                                </div>
                            </div>
                        ...
                        <div className="bg-foreground p-10 text-background">
                            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                                <div>
                                    ...
                                    <p className="text-text-muted mt-1">Sesión #{registerData.shortId} • Operación Exitosa</p>
                                </div>
                                <div className="text-center bg-card/10 p-4 rounded-xl border border-white/10">
                                    <p className="text-xs text-text-muted uppercase">Fondo Base</p>
                        ...
                        <div className="p-10 space-y-10">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-4">
                                    <h3 className="text-sm font-bold text-text-muted uppercase tracking-wider">Ventas por Método</h3>
                                    <div className="space-y-2">
                                        {Object.entries(registerData.breakdown || {}).map(([method, total]: [string, any]) => (
                                            <div key={method} className="flex justify-between p-4 bg-surface-alt rounded-xl border border-border-subtle">
                                                <span className="font-medium">{method}</span>
                                                <span className="font-bold">${Number(total).toLocaleString()}</span>
                                            </div>
                                        ))}
                                        <div className="p-4 bg-foreground text-background rounded-xl flex justify-between items-center mt-4">
                            ...
                            <div className="flex flex-col md:flex-row justify-center gap-4 pt-8 border-t border-border-subtle">
